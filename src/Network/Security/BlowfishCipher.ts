...
const ConstantSInit = new Uint32Array([
...
class BlowfishCipher {
  ...
  public setIVs(decryptionIV: Uint8Array, encryptionIV: Uint8Array): void {
    this.decryptionIV = decryptionIV.slice();
    this.encryptionIV = encryptionIV.slice();
    this.decryptCount = 0;
    this.encryptCount = 0;
  }
  ...
  private n21(iv: Uint8Array, x: number): number {
    return (iv[x] << 24) | (iv[x + 1] << 16) | (iv[x + 2] << 8) | iv[x + 3];
  }

  private n12(iv: Uint8Array, x: number, v: number): void {
    iv[x] = (v >> 24) & 0xff;
    iv[x + 1] = (v >> 16) & 0xff;
    iv[x + 2] = (v >> 8) & 0xff;
    iv[x + 3] = v & 0xff;
  ...
  private encipherBlock(block: Uint32Array): void {
    let lv = block[0];
    let rv = block[1];
